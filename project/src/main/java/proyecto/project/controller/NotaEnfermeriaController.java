...
public String guardarNotaEnfermeria(
    ...
    // Concatenar medicamento y dosis
    List<String> medicamentosConcatenados = new ArrayList<>();
    for (int i = 0; i < medicamentos.size(); i++) {
        String medicamento = medicamentos.get(i);
        String dosisMedicamento = dosis.get(i);
        medicamentosConcatenados.add(medicamento + " (" + dosisMedicamento + " mg)");
    }

    // Unir los medicamentos concatenados en una cadena separada por comas
    String medicamentoFormato = String.join(", ", medicamentosConcatenados);

    // Guardar en la base de datos usando el servicio
    notaEnfermeriaService.guardarNotaEnfermeria(
            fcBpm, paMmhg, tempC, spo2Pct, frRpm, gluMgdl,
            identificacionP, idProfesional, actividad, medicamentoFormato, notaEnfer
    );
